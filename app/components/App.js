...
export default class App extends React.Component {
  constructor() {
    ...
    this.state = {
      ...
      currentTabIndex: storage.getItem("currentTabIndex")
        ? parseInt(storage.getItem("currentTabIndex"))
        : 0,
      tabs: storage.getItem("tabs")
        ? JSON.parse(storage.getItem("tabs"))
        : [
            {
              name: null,
              headers: {},
    ...
    if (endpoint == "") {
      return Promise.resolve({
        ...
        errors: [
          {
            message:
              "Provide a URL to a GraphQL endpoint to start making queries to it!",
          },
        ],
      });
    }
  ...
  render() {
        ...
        <div className="graphiql-wrapper">
          ...
          <GraphiQL
            ...
            key={
              currentTabIndex +
              currentTab.endpoint +
              JSON.stringify(currentTab.headers)
            }
    ...
    return (
      <div className="wrapper">
            ...
            {_.map(this.state.tabs, (tab, tabIndex) => {
              return (
                <li
                  key={tabIndex}
                  className={
                    tabIndex === this.state.currentTabIndex ? "active" : ""
                  }
                >
                  <a
                    href="javascript:;"
        ...
        <div className="tabs">{tabEl}</div>
        <Modal
          isOpen={this.state.headerEditOpen}
          onRequestClose={this.closeModal}
        >
          <HTTPHeaderEditor
            headers={_.map(
              this.state.tabs[this.state.currentTabIndex].headers,
              (value, key) => ({ key, value })
            )}
